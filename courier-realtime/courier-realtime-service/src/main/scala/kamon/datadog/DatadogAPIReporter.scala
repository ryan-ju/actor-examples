...
class DatadogAPIReporter extends MetricReporter {
  ...
  private def buildRequestBody(snapshot: TickSnapshot): String = {
    ...
    def addDistribution(metric: MetricDistribution): Unit = {
      ...
      addMetric(name + ".p99", valueFormat.format(scale(distribution.percentile(99D).value, unit)), "gauge", metric.tags)
      addMetric(name + ".p999", valueFormat.format(scale(distribution.percentile(99.9D).value, unit)), "gauge", metric.tags)
